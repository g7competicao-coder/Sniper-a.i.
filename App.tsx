...
const App: React.FC = () => {
  ...
  // FIX: The `new Date()` constructor was called without arguments. Initializing with `Date.now()` to satisfy a strict linting rule.
  ...
  const fetchSignals = useCallback(async () => {
    if (signals.length === 0 && !isConnectionError) {
        setLoading(true);
    ...
    if (connectionHasError) {
        setError("Sem conexão com a Binance");
  ...
  }, [signals, historicalSignals, isConnectionError]);
  ...
  const renderSignalCards = () => {
    if (loading && signals.length === 0 && !isConnectionError) {
      return (
    ...
    if (isConnectionError) {
      return (
        <div className="text-center py-10 px-4 bg-gray-800 rounded-lg flex flex-col items-center justify-center min-h-[540px]">
          <p className="text-brand-red font-semibold text-lg">Atenção</p>
          <p className="text-gray-400 mt-2 mb-6">{error}</p>
          <button
            onClick={() => fetchSignals()}
            disabled={loading}
            className="bg-brand-blue hover:bg-brand-blue/80 text-white font-bold py-3 px-8 rounded-lg transition-colors duration-200 disabled:bg-gray-600 disabled:cursor-not-allowed flex items-center justify-center min-w-[180px]"
          >
            {loading ? (
              <>
    ...
    if (error && signals.length === 0) {
      return (
        <div className="text-center py-10 px-4 bg-gray-800 rounded-lg min-h-[540px] flex flex-col items-center justify-center">
            <InfoIcon className="h-12 w-12 text-brand-blue mb-4"/>
            <p className="text-gray-200 font-semibold text-lg">Buscando Oportunidades</p>
  ...
  return (
    <div className="h-screen p-2">
      <div className="h-full w-full bg-gray-900 text-gray-200 rounded-2xl overflow-y-auto no-scrollbar">
        <Header 
